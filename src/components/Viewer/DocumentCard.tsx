  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!success && !navigator.share) {
        showToast('Обмен файлами не поддерживается', 'warning')
      }
    } catch (err) {
    ...
    >
      ...
      <div className="relative aspect-[3/4] bg-slate-700 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
          ...
          <button
            onClick={handleShare}
            className="w-10 h-10 rounded-full bg-emerald-600 hover:bg-emerald-500 flex items-center justify-center text-white transition-colors shadow-lg shadow-emerald-900/40"
            disabled={isSharing}
